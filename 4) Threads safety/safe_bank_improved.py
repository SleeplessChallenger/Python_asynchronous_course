...
def main():
	...
	total = sum(bal.balance for bal in accounts)
	...
	print(f'Transfers complete {dt.total_seconds()}')
	validate_bank(accounts, total)
...
def do_transfer_bad_example(from_account: Account, to_account: Account, amount: int):
	if from_account.balance < amount:
		return
...
##################### Code below is the one that causes errors
transfer_lock = RLock()

def do_transfer_old_version(from_account: Account, to_account: Account, amount: int):
	if from_account.balance < amount:
		return
...
def validate_bank(accounts: List[Account], total: int, quiet=False):
	...
	if current != total:
		print(f'Error: Inconsistent account balance: {current} {total}',
			  flush=True)
	elif not quiet:
		print(f'All good: Consistent account balance {total}',
			  flush=True)
