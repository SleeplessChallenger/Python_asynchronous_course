...
def main():
	...
	threads = [
		threading.Thread(target=generate_data, args=(7, data), daemon=True),
		threading.Thread(target=generate_data, args=(7, data), daemon=True),
		threading.Thread(target=process_data, args=(14, data), daemon=True)
	]
	...
	print(colorama.Fore.WHITE +
		  'App exiting, total time: {:,.2f} sec.'
		  .format(dt.total_seconds()), flush=True)
...
def generate_data(num: int, data: list):
	for idx in range(1, num + 1):
		...
		print(colorama.Fore.YELLOW + f"-- generated item {idx}",
			  flush=True)
...
def process_data(num: int, data: list):
	...
	while processed < num:
		...
		print(colorama.Fore.CYAN +
			  " +++ Processed value {} after {:,.2f} sec."
			  .format(value, dt.total_seconds()), flush=True)
		time.sleep(.5)
